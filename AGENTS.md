...
- `scripts/scrape.ts` — the scraper. Reads both official sources over plain HTTP; see
  `scripts/scraper.md` for the endpoints and their failure modes. Outside the `tsconfig.json`
  `include`, so `pnpm typecheck` never sees it, but `scripts/__tests__/` does cover it.
